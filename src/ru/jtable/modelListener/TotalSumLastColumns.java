...
public class TotalSumLastColumns {
    ...
    public void getSum(JBroTable table, int row, int column, String typeOfStatement) {
        ...
        if (columnToModelField.getIdentifier().startsWith("ФЕ Итого1") && columnTemp > 0) {
            ...
            model.setValueAt(
                    ...
                    row,
                    getColumnFromIdentifier(table, "ПЕ Всего"));
            model.setValueAt(fmt(sum), row, columnTemp); // устанавливаем значение в ячейку Итого для ФЕ, округленное до 2 знаков после запятой 
        }
        if (columnToModelField.getIdentifier().startsWith("ФЕ Итого2") && columnTemp > 0) {
        ...
        if (columnToModelField.getIdentifier().startsWith("ФЕ Итого3") && columnTemp > 0) {
            Object data = model.getValueAt(row, column);
        ...
        if (columnToModelField.getIdentifier().startsWith("ФЕ Итого4") && columnTemp > 0) {
            Object data = model.getValueAt(row, column);
    ...
    // Получаем столбец с требуемым идентификатором
